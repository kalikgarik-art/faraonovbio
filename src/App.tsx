...
import { registerVisit } from './services/views';
...
export default function App() {
  const [config, setConfig] = useState<BioConfig>(() => {
    try {
      ...
      if (saved) {
        ...
        // Migrate legacy overlay texts or UID if user had old cached config
        if (parsed.theme?.enterScreenText === '[ faraonov.lol ]') {
          parsed.theme.enterScreenText = 'тыкай';
        }
        if (parsed.theme?.enterScreenSubtext === 'click anywhere to enter') {
          parsed.theme.enterScreenSubtext = 'био фараончика.';
        }
        if (parsed.profile?.userUid === '#38073') {
          parsed.profile.userUid = '';
        }
        return parsed;
      }
    } catch {
  ...
  const [liveViews, setLiveViews] = useState<number>(config.profile.viewsCount);

  // Register real visitor view count on load (1 view per unique person)
  useEffect(() => {
    let isMounted = true;
    registerVisit(config.profile.viewsCount).then((count) => {
      if (isMounted) {
        setLiveViews(count);
      }
    });
  ...
  return (
    <main className="relative min-h-screen w-full overflow-hidden bg-black text-white flex flex-col items-center justify-center select-none selection:bg-white/20 selection:text-white">
      ...
      <ClickToEnter
        ...
        onEnter={handleEnter}
        title={config.theme.enterScreenText || 'тыкай'}
        subtitle={config.theme.enterScreenSubtext || 'био фараончика.'}
      />
